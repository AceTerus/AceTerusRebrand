...
import {
  ...
  Layers, Loader2, PenLine, ScanLine, Sparkles, Star, Target, Trophy, X, XCircle, Zap,
} from "lucide-react";
...
const Quiz = () => {
  ...
  const filteredDecks = useMemo(() => decks.filter((d) => (d.subject ?? "General") === selectedCategory), [decks, selectedCategory]);
  ...
  return (
    ...
    >
      ...
      <style>{`
        ...
        @keyframes atl-answer-bounce {
          0%   { transform: scale(1); }
          40%  { transform: scale(1.03) translateY(-3px); }
          100% { transform: scale(1) translateY(0); }
        }
        .atl-answer-bounce { animation: atl-answer-bounce 0.3s cubic-bezier(0.34,1.56,0.64,1); }
        @keyframes atl-q-in {
          from { opacity:0; transform:translateX(20px); }
          to   { opacity:1; transform:translateX(0); }
        }
        .atl-q-in { animation: atl-q-in 0.4s cubic-bezier(0.22,1,0.36,1) both; }
        .atl-nav-link { display:inline-block; transition:transform 0.18s cubic-bezier(0.34,1.56,0.64,1),color 0.15s; }
      ...
      <div className="relative z-[2] max-w-6xl mx-auto px-5 pt-10">

        {/* ── HEADER ── */}
        <div className="text-center mb-10 atl-fade-up">
          <img src={Logo} alt="AceTerus" className="w-16 h-16 rounded-[22px] border-[3px] border-[#0F172A] shadow-[5px_5px_0_0_#0F172A] mx-auto mb-4 atl-logo" />
          <h1 className={`${DISPLAY} font-extrabold text-4xl md:text-5xl leading-none`}>Quiz Arena 🎯</h1>
          <p className="mt-2 font-medium text-slate-500">Practice with authentic exam papers and track your progress</p>
          <button onClick={() => setShowGoalSheet(true)} className={`${BTN} mt-5 text-white`} style={{ background: C.blue }}>
            <CalendarDays className="w-4 h-4" /> My Goals
          </button>
        </div>
        ...
        <div className="grid grid-cols-2 md:grid-cols-4 gap-5 mb-10">
          {statCards.map(({ Icon, value, label, bg, ic }, i) => (
            <div key={label} className={`${STICKER_SM} p-5 text-center atl-fade-up hover:-translate-y-1 hover:shadow-[6px_8px_0_0_#0F172A]`} style={{ animationDelay: `${80 + i * 70}ms` }}>
              <div className="w-12 h-12 rounded-[16px] border-[2.5px] border-[#0F172A] shadow-[3px_3px_0_0_#0F172A] flex items-center justify-center mx-auto mb-3" style={{ background: bg }}>
                <Icon className="w-6 h-6" style={{ color: ic }} />
              </div>
              <div className={`${DISPLAY} font-extrabold text-3xl leading-none`}>{value}</div>
              <div className="font-bold text-xs text-slate-500 mt-1">{label}</div>
            </div>
          ))}
        </div>

        {deckError && (
          <div className={`${STICKER_SM} p-5 mb-8 border-l-[6px]`} style={{ borderLeftColor: C.pop }}>
        ...
        {view === "categories" && (
          <div className="mb-10">
            {/* OMR Scanner banner */}
            <div className={`${STICKER_SM} p-5 mb-8 flex flex-col sm:flex-row items-start sm:items-center gap-4 atl-fade-up`} style={{ animationDelay: "200ms", background: C.indigoSoft }}>
              <div className="w-12 h-12 rounded-[16px] border-[2.5px] border-[#0F172A] shadow-[3px_3px_0_0_#0F172A] flex items-center justify-center shrink-0" style={{ background: C.indigo }}>
                <ScanLine className="w-6 h-6 text-white" />
              </div>
              <div className="flex-1 min-w-0">
                <p className={`${DISPLAY} font-extrabold text-base`}>OMR Answer Sheet Scanner</p>
                <p className="text-sm font-medium text-slate-600 mt-0.5">Upload a filled answer sheet to grade it instantly with optical mark recognition.</p>
              </div>
              <button className={`${BTN_SM} text-white shrink-0`} style={{ background: C.indigo }} onClick={() => navigate("/omr-scan")}>
                <ScanLine className="w-4 h-4" /> Open Scanner
              </button>
            </div>

            <h2 className={`${DISPLAY} font-extrabold text-3xl mb-6`}>Pick a subject 📚</h2>

            {loadingDecks ? (
              <div className="grid grid-cols-1 md:grid-cols-2 xl:grid-cols-3 gap-6">
                {Array.from({ length: 6 }).map((_, i) => (
                  <div key={i} className={`${STICKER} p-6 space-y-3`}>
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 xl:grid-cols-3 gap-6">
                {enrichedCategories.map((cat, i) => {
                  ...
                  return (
                    <div key={cat.name} className={`${STICKER_CARD} flex flex-col overflow-hidden atl-fade-up`} style={{ padding: 0, animationDelay: `${i * 80}ms` }}>
                      {catImage ? (
                        <div className="relative h-40 shrink-0 rounded-t-[25px] overflow-hidden">
                          <img src={catImage} alt={cat.name} className="w-full h-full object-cover" />
                          <div className="absolute inset-0 bg-gradient-to-t from-[#0F172A]/75 via-black/20 to-transparent" />
                          <div className="absolute bottom-3 left-4 flex items-center gap-2">
                            <BookOpenCheck className="w-5 h-5 text-white" />
                            <h3 className={`${DISPLAY} font-extrabold text-lg text-white drop-shadow`}>{cat.name}</h3>
                          </div>
                        </div>
                      ) : (
                        <div className="p-6 pb-0 flex items-center gap-3">
                          <div className="w-10 h-10 rounded-[14px] border-[2.5px] border-[#0F172A] shadow-[3px_3px_0_0_#0F172A] flex items-center justify-center" style={{ background: C.cyan }}>
                            <BookOpenCheck className="w-5 h-5" style={{ color: C.ink }} />
                          </div>
                          <h3 className={`${DISPLAY} font-extrabold text-xl`}>{cat.name}</h3>
                        </div>
                      )}
                      <div className="p-6 flex-1 flex flex-col gap-4">
                        {cat.description && <p className="text-sm font-medium text-slate-500">{cat.description}</p>}
                        {cat.decks.length === 0 ? (
                          <p className="text-sm font-medium text-slate-400 italic mt-auto">Quizzes coming soon.</p>
                        ) : (
                          <>
                            <div className="flex flex-wrap gap-3">
                              <span className={`${TAG}`} style={{ background: C.skySoft }}>
                                <BookOpen className="w-3 h-3" /> {cat.decks.length} {cat.decks.length === 1 ? "quiz" : "quizzes"}
                              </span>
                              <span className={`${TAG}`} style={{ background: C.blueSoft }}>
                                <Layers className="w-3 h-3" /> {cat.totalQuestions.toLocaleString()} questions
                              </span>
                            </div>
                            <button className={`${BTN} w-full justify-center text-white mt-auto`} style={{ background: C.blue }}
                              onClick={() => { setSelectedCategory(cat.name); setView("decks"); }}>
                              View Quizzes <ChevronRight className="w-4 h-4" />
                            </button>
                          </>
                        )}
                      </div>
        ...
        {view === "decks" && (
          <div className="mb-10">
            <div className="flex items-center gap-4 mb-8">
              <button className={`${BTN_SM} bg-white`} onClick={() => setView("categories")}>
                <ChevronLeft className="w-4 h-4" /> Back
              </button>
              <h2 className={`${DISPLAY} font-extrabold text-3xl`}>{selectedCategory}</h2>
            </div>

            {quizError && (
              <div className={`${STICKER_SM} p-5 mb-6`} style={{ borderLeft: `6px solid ${C.pop}` }}>
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 xl:grid-cols-3 gap-6">
              {filteredDecks.map((deck, i) => (
                <div key={deck.id} className={`${STICKER_CARD} p-6 flex flex-col gap-4 atl-fade-up`} style={{ animationDelay: `${i * 80}ms` }}>
                  <div>
                    <h3 className={`${DISPLAY} font-extrabold text-xl`}>{deck.name}</h3>
                    {deck.subject && (
                      <span className={`${TAG} mt-2`} style={{ background: C.skySoft }}>{deck.subject}</span>
                    )}
                  </div>
                  <p className="text-sm font-medium text-slate-500 flex-1">{deck.description ?? "No description provided."}</p>
                  ...
                  >
                    {quizLoading && activeDeck?.id === deck.id
                      ...
                      : <>Start Quiz <Zap className="w-4 h-4" /></>}
                  </button>
                </div>
              ))}
            </div>
            ...
            {filteredDecks.length === 0 && (
              <div className={`${STICKER} p-10 text-center mt-6`}>
                <p className={`${DISPLAY} font-extrabold text-xl`}>No quizzes here yet.</p>
              </div>
            )}
          </div>
        )}
        ...
        {view === "taking" && (
          <div className="space-y-6 mb-10">
            ...
            {!quizLoading && !quizError && quizPayload && questions.length > 0 && sessionComplete && (
              <div className="space-y-6 atl-fade-up">
                ...
                {/* Goals CTA */}
                <button onClick={() => setShowGoalSheet(true)} className={`${BTN} w-full justify-center text-white`} style={{ background: C.blue }}>
            ...
            {!quizLoading && !quizError && quizPayload && questions.length > 0 && !sessionComplete && currentQuestion && (
              <div className={`${STICKER} p-6 md:p-8`}>
                ...
                {/* Progress bar */}
                <div className="h-3.5 rounded-full border-[2.5px] border-[#0F172A] bg-slate-100 overflow-hidden shadow-[2px_2px_0_0_#0F172A] mb-6">
                ...
                {/* Question body — key forces re-animation on navigation */}
                <div key={`q-${currentIndex}`} className="atl-q-in space-y-6">
                          ...
                          {currentQuestion.answers.map((a) => {
                            ...
                            return (
                              <button key={a.id} onClick={() => handleCheckboxToggle(a.id)}
                                className={cn("w-full rounded-[20px] border-[3px] border-[#0F172A] p-4 text-left shadow-[4px_4px_0_0_#0F172A] transition-all duration-150 hover:-translate-y-0.5 hover:shadow-[4px_4px_0_0_#0F172A] active:translate-y-0 active:shadow-[2px_2px_0_0_#0F172A] flex items-center gap-4",
                          ...
                          return (
                            <button
                              ...
                              className={cn(
                                ...
                                "hover:-translate-y-0.5 hover:shadow-[4px_4px_0_0_#0F172A]",
                ...
                {submitConfirmPending && (
                  <div className="rounded-[20px] border-[3px] border-[#0F172A] shadow-[5px_5px_0_0_#0F172A] p-5 mt-6" style={{ background: "#fff7ed" }}>
                ...
                {/* Navigation */}
                <div className="flex flex-wrap items-center gap-3 pt-5 mt-4 border-t-[2px] border-slate-100">
